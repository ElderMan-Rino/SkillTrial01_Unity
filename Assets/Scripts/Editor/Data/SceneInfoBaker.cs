...
namespace Elder.SkillTrial.Resources.Data
{
	public static class SceneInfoBaker
	{
		public static void Bake(string sourcePath, string savePath, byte[] encryptionKeyPartB)
		{
			...
			var dtoList = rawList.Select(row => new SceneInfo(row[0]?.ToString() ?? string.Empty, row[1]?.ToString() ?? string.Empty, System.Convert.ToInt32(row[2]), (SceneLoadType)System.Convert.ToInt32(row[3]))).ToList();
