...
{
	public static class SplashEntryInfoBaker
	{
		public static void Bake(string sourcePath, string savePath, byte[] encryptionKeyPartB)
		{
			...
			var dtoList = rawList.Select(row => new SplashEntryInfo(row[0]?.ToString() ?? string.Empty, System.Convert.ToInt32(row[1]), System.Convert.ToSingle(row[2]))).ToList();

			var builder = new BlobBuilder(Allocator.Temp);
			ref SplashEntryInfoRoot root = ref builder.ConstructRoot<SplashEntryInfoRoot>();
			var arrayBuilder = builder.Allocate(ref root.Rows, dtoList.Count);

			for (int i = 0; i < dtoList.Count; i++)
			{
				builder.AllocateString(ref arrayBuilder[i].SpriteName, dtoList[i].SpriteName);
				arrayBuilder[i].Id = dtoList[i].Id;
				arrayBuilder[i].Interval = dtoList[i].Interval;
			}

			var blobRef = builder.CreateBlobAssetReference<SplashEntryInfoRoot>(Allocator.Temp);
			builder.Dispose();
